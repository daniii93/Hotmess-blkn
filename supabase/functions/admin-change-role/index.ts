...
serve(async (request) => {
  ...
  const { data: before } = await supabase.from("profiles").select("id,role").eq("id", body.user_id).maybeSingle();
  const { error } = await supabase.from("profiles").update({ role: body.role }).eq("id", body.user_id);
  if (error) return json({ error: error.message }, 500);

  await audit(supabase, body.admin_id, "role_change", "user", body.user_id, before, { role: body.role }, body.reason);
