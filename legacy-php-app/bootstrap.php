...
// Wartungsmodus — maintenance.flag im Webroot aktiviert den Modus.
// Admins (user_role = admin in Session) werden immer durchgelassen.
if (file_exists(dirname(__DIR__) . '/maintenance.flag')) {
    $uri = strtok($_SERVER['REQUEST_URI'] ?? '/', '?');
    $isAdmin = ($_SESSION['user_role'] ?? '') === 'admin';
    $isLoginPage = in_array($uri, ['/login.php', '/login', '/logout.php', '/logout'], true);
    ...
    if (!$isAdmin && !$isLoginPage) {
        if (str_starts_with($uri, '/api/')) {
            ...
            echo json_encode(['error' => 'Wartungsmodus', 'maintenance' => true]);
            exit;
        }
        http_response_code(503);
        header('Retry-After: 3600');
...
require_once __DIR__ . '/phase1.php';

function hotmess_ensure_auth_schema(): void
{
    $columns = [
        'username' => 'ALTER TABLE users ADD COLUMN username VARCHAR(80) NULL UNIQUE AFTER email',
        'birth_date' => 'ALTER TABLE users ADD COLUMN birth_date DATE NULL AFTER last_name',
        'house_number' => 'ALTER TABLE users ADD COLUMN house_number VARCHAR(40) NULL AFTER street',
        'avatar_url' => 'ALTER TABLE users ADD COLUMN avatar_url VARCHAR(255) NULL AFTER country',
        'bio' => 'ALTER TABLE users ADD COLUMN bio TEXT NULL AFTER avatar_url',
        'email_verified' => 'ALTER TABLE users ADD COLUMN email_verified TINYINT(1) NOT NULL DEFAULT 0 AFTER email_verified_at',
    ];
...
require_once __DIR__ . '/referral-data.php';
require_once __DIR__ . '/checkin-data.php';
