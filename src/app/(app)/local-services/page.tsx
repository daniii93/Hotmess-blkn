...
import { BadgeCheck, BriefcaseBusiness, ChevronRight, ShieldCheck, Wrench } from "lucide-react";
  ...
  return (
    <main className="mx-auto grid w-full max-w-6xl gap-8 px-4 pb-24 pt-8 sm:px-6 lg:px-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.6fr_0.4fr] lg:items-end">
          ...
          <div className="grid gap-3">
            ...
            <Link href="/local-services/company/activate" className="inline-flex items-center justify-between rounded-pill border border-hm-gold/35 px-5 py-3 text-sm font-bold text-hm-ink">
              Als Anbieter starten <ChevronRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <InfoCard icon={<BadgeCheck className="h-5 w-5" />} title="Kontakt geschuetzt" text="Telefon, E-Mail und externe Links werden vor Leadkauf und Beauftragung gefiltert." />
        <InfoCard icon={<BriefcaseBusiness className="h-5 w-5" />} title="Business-Modul" text="Anbieter benoetigen ein geprueftes Business-Profil und die Modulfreigabe local_services." />
      </section>
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            {categories.slice(0, 12).map((category) => (
              <Link key={category.id} href={`/local-services/create?category=${category.id}`} className="rounded-xl border border-hm-border bg-hm-ivory px-4 py-3 hover:border-hm-gold/50">
                ...
                <span className="mt-1 block text-xs text-hm-inkSoft">Lead ab {formatLocalServiceMoney(category.minLeadPriceCents)}</span>
              </Link>
            ))}
          </div>
        </div>
  ...
  if (me.providerProfile?.verificationStatus === "verified") {
    return (
      <div className="rounded-card border border-emerald-200 bg-emerald-50 p-5 shadow-luxury">
        ...
        <p className="mt-2 text-sm text-emerald-700">Du kannst Leads kaufen und Angebote senden.</p>
